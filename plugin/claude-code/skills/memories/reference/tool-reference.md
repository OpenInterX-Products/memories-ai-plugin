...
### `upload_video_from_file(path, unique_id?, callback?, tags?, video_transcription_prompt?, datetime_taken?, camera_model?, latitude?, longitude?, retain_original_video?, wait_for_indexing?, timeout_seconds?)`
Index a **local** video file by uploading its bytes (multipart) to `/serve/api/v1/upload`. **Reads from
the MCP server's own filesystem** — only works when the server runs on the same machine as the file
(local / stdio); a remote deployment can't read your local files (use `upload_video_from_url` for those).
- `callback` — optional URL memories.ai POSTs to when indexing finishes · `retain_original_video` — keep
  the original uploaded file · `tags` / `datetime_taken` (`yyyy-MM-dd HH:mm:ss`) / `camera_model` /
  `latitude` / `longitude` / `video_transcription_prompt` — same meaning as in `upload_video_from_url`.
- `wait_for_indexing` (default true) — poll until `PARSE`/`FAIL` or `timeout_seconds` (default 180, ≤540).
- Index under a `unique_id` and search with the **same** one.
